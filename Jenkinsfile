  ...
  stages {
    ...
    stage('Release docker image for development') {
      ...
      steps {
        ...
        sh(returnStdout: true, script: '''
          images=`docker images | grep entropypool | grep user-management | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep user-management | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/user-management/k8s/01-user-management.yaml'
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/user-management:latest/user-management:$tag/g" cmd/user-management/k8s/01-user-management.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/user-management/k8s/01-user-management.yaml
